...
#define __RECV_BUF__ (8192)
	void CRupEndpoint::handle_input_evt()
	{
		...
		m_ui64RecvCnt++;
		int32_t i32Ret = m_pSock->handle_can_recv(__RECV_BUF__);
...
#define __MAX_CYCLE_CNT__ (10000000)
		...
		if (ES_OPENED == m_sm.get_cur_state()) ///如果发生了错误，导致状态不是OPENNED时，就不用处理IO了。
		{
			...
			else
			{
				...
				if (++m_ui32CycleCnt > __MAX_CYCLE_CNT__)
				{
					...
					{
						TRACE_LOG(m_log, ELL_DEBUG, "send hb!\n");
					...
					if (m_ui64RecvCnt == m_ui64RecvStore)
					{
						...
						{
							///cut it down
							TRACE_LOG(m_log, ELL_DEBUG, "keepalive failed\n");
							m_sm.post_evt(EE_INTERNAL_ERR, NULL);
						}
					}
					else
	...
	int32_t CRmpEndpoint::close()
	{
		return m_sm.post_evt(EE_CLOSE, NULL);
	}

	int32_t CRmpEndpoint::send_data(nm_mem::mem_ptr_t &pData)
	{
		return m_sm.get_cur_state() == ES_OPENED ? m_pSock->send(pData) : CMNERR_COMMON_ERR;
	}
	 ...
	 *
	 * */
#define __INIT_SPEED__ (500)
#define __KEEP_ALIVE__ (1000000)
#define __SENDER_HB_INTERVAL__ (1000000)
	int32_t CRmpEndpoint::handling_closed_to_adding_into_ot(int32_t i32CurState, int32_t i32Evt, int32_t i32NextState, cmn_pvoid_t pVoid)
	{
		...
		do
		{
			...
			i32Ret = m_pSock->open(m_strMulticastIp, m_ui16BindPort, m_ui8SenderId, m_ui32AckConfirmCnt, __KEEP_ALIVE__, __INIT_SPEED__, __SENDER_HB_INTERVAL__);
			if (CMNERR_SUC != i32Ret)
			{
				break;
			}
	 ...
	 *
	 * */
	void CRmpEndpoint::handle_input_evt()
	{
		if (ES_OPENED != m_sm.get_cur_state()) ///有可能INPUT 处理线程先设置完毕，并开始接受数据，但是发送线程还在设置过程中，所以epoll要用level triger
		{
			return;
		...
		int32_t i32Ret = m_pSock->handle_can_recv(__RECV_BUF__);
		if (CMNERR_ODATA_READY == i32Ret)
		{
			mem_ptr_t pMem;
			while (CMNERR_SUC == m_pSock->get_next_recved_data(pMem))
			{
				on_recved_data(pMem);
			}
		}
	 ...
	 *
	 * */
	sock_handle_t CRmpEndpoint::get_ioobj_handle()
	{
		return m_pSock->get_handle();
	}
