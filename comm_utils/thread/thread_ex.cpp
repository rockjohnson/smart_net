...
namespace nm_thread
{
		...
		m_bStop(false), m_thread(0)
	{
		...
		LOG_ASSERT(RET_SUC == pthread_attr_init(&m_attr)); ///The  pthread_attr_init()  function  initializes  the  thread attributes object pointed to by attr with default attribute values.
#else
#endif
	}
	...
	{
#if __PLATFORM__ == __PLATFORM_LINUX__
		LOG_ASSERT(RET_SUC == pthread_attr_destroy(&m_attr)); ///The  pthread_attr_init()  function  initializes  the  thread attributes object pointed to by attr with default attribute values.
#else
#endif
	}
